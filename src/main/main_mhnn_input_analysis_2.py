...
class MetaParams:
    ...
    evaluate_sparsity_similarity: bool = True
    evaluate_distribution: bool = False
...
class ExpMHNNBTSPFinetune(auto_experiment.ExperimentInterface):
    ...
    def execute_experiment_process(self, parameters: MetaParams, dataset):
        ...
        with torch.no_grad():
            ...
            # modalities(without snn): vision, position, time, direction
            modalities = [vision_all, position_all, time_all, direction_all]
            modality_names = ['vision', 'position', 'time', 'direction']
            
            
            if parameters.evaluate_sparsity_similarity:
                # this time, plot the sparsity distribution and dice coefficient distribution against each modality

                # plot all sparsity and dice coefficient distributions in one figure
                # col: sparsity, dice coefficient
                fig, ax = plt.subplots(len(modalities), 2, figsize=(12, 24))
                ...
                for modality_idx in range(len(modalities)):
                    ...
                    if parameters.flatten_temporal_input:
                        ...
                        modality = modality.permute(1, 0, 2).reshape(total_len, -1)
                        assert modality.size(0) == total_len
                    else:
                        flatten_flag = 'not_flatten'
                        modality = modality.permute(1, 0, 2).reshape(-1, modality.size  (2))
                        assert modality.size(0) == total_len * seq_len_aps

                    # calculate the sparsity of each input
                    curr_mod_sparsity = torch.sum(modality, dim=-1) / modality.size(-1)

                    # plot the sparsity distribution
                    # get axis
                    curr_ax_sparsity = ax[modality_idx][0]
                    sns.histplot(curr_mod_sparsity.cpu().numpy(),   ax=curr_ax_sparsity)
                    # curr_ax_sparsity.set_xlim(0, 1)
                    curr_ax_sparsity.set_xlabel('Sparsity')
                    curr_ax_sparsity.set_ylabel('Count')
                    curr_ax_sparsity.set_title(f'{modality_name} ({flatten_flag})')

                    # calculate the dice coefficient between each pair of inputs
                    dc_matrix = dice_coefficient_matrix(modality)
                    ...
                    # get axis
                    curr_ax_dice = ax[modality_idx][1]
                    sns.histplot(dc_matrix.cpu().numpy(), ax=curr_ax_dice)
                    # curr_ax_dice.set_xlim(0, 1)
                    curr_ax_dice.set_xlabel('Dice Coefficient')
                    curr_ax_dice.set_ylabel('Count')
                    curr_ax_dice.set_title(f'{modality_name} ({flatten_flag})')

                # save the figure
                fig.tight_layout()
                fig.savefig(os.path.join(self.experiment_folder,    f'sparsity_dice_{train_exp_idx[0]}_{flatten_flag}.svg'))

                # close the figure
                plt.close(fig)
            
            if parameters.evaluate_distribution:
                # this time, plot the element distribution of each modality
                # plot the distribution of each modality in one figure
                fig, ax = plt.subplots(len(modalities), 1, figsize=(6, 24))
                sns.set_style("darkgrid")
                for modality_idx in range(len(modalities)):
                    modality: torch.tensor = modalities[modality_idx]
                    modality_name = modality_names[modality_idx]

                    # reshape modality to a 1D tensor
                    modality = torch.flatten(modality)
                    
                    # calculate mean and std
                    mean = torch.mean(modality)
                    std = torch.std(modality)
                    ...
                    # plot the distribution of all elements
                    # get axis
                    curr_ax = ax[modality_idx]
                    sns.histplot(modality.cpu().numpy(), ax=curr_ax)
                    curr_ax.set_xlabel('Value')
                    curr_ax.set_ylabel('Count')
                    curr_ax.set_title(f'{modality_name} ({flatten_flag}), mean: {mean:.2f}, std: {std:.2f}')
                    
                # save the figure
                fig.tight_layout()
                fig.savefig(os.path.join(self.experiment_folder, f'distribution_{train_exp_idx[0]}_{flatten_flag}.svg'))
...
if __name__ == "__main__":
    ...
    meta_params = MetaParams(
        ...
        experiment_index=0,
        experiment_name="mission_5_btsp_input_distribution",
        evaluate_distribution=True,
        evaluate_sparsity_similarity=False,
    )
